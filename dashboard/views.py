...
def dashboard(request):
    """
    Displays different dashboards for employers and job seekers.
    ...
    if request.user.profile.role == 'employer':
        ...
        return render(
            request,
            'dashboard/dashboard-employer.html',
            {'user_jobs': user_jobs, 'job_applications': job_applications}
        )
    ...
    # If the user is a job seeker, display the job seeker dashboard
        ...
        return render(
            request, 'dashboard/dashboard-jobseeker.html',
...
def view_application(request, application_id):
    """
    The logic allows both job seekers and employers to view the application.
    Job seekers can view their own applications,
    while employers can view applications for the jobs they posted.
    """
...
def view_application_employer(request, application_id):
    """
    Allows the employer to accept or reject applications.
    Employers can view details of each application for the jobs they posted,
    and can take action to accept or reject applications.
    """

    # Ensure the application belongs to a job posted by the logged-in employer
    application = get_object_or_404(
        Application, id=application_id, job__employer=request.user
    )
    ...
    if request.method == 'POST':
        ...
        if 'accept' in request.POST:
            ...
            messages.success(
                request,
                f'Application from {application.applicant.username} '
                'has been accepted.'
            )
        ...
        return redirect(
            'view_application_employer', application_id=application.id)
...
def withdraw_application(request, application_id):
    ...
    Job seekers can confirm and withdraw an application.
    """
    ...
    return render(
        request, 'dashboard/withdraw_application.html',
        {'application': application}
    )
